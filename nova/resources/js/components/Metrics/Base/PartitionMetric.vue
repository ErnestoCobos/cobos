...
export default {
    ...
    computed: {
        ...
        formattedItems()
        {
            return _(this.chartData).map((item, index) => {
                return {
                    label: item.label,
                    value: item.value,
        ...
        formattedLabels()
        {
            return _(this.chartData).map(item => item.label).value();
        },
        ...
        formattedTotal()
        {
            ...
            if (roundedTotal.toFixed(2) == total) {
                return Nova.formatNumber(String(roundedTotal));
            }

            return Nova.formatNumber(String(total));
        },
